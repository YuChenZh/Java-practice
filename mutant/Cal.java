/**
 * Created by yujingchen on 11/29/17.
 */
import java.io.*;
...
public class Cal
{
                           ...
                           int day2, int year)
    {
        ...
        if (month2 == month1) // in the same month
            numDays  = day2 - day1;
//          numDays  = Math.abs(day2 - day1); // 1.ABS –– Absolute Value Insertion
//          numDays  = day2 + day1; // 2. AOR –– Arithmetic Operator Replacement
        ...
        else
        {
            ...
            int m4 = year % 4;
//          int m4  = year / 4; // mutant

//            int m100 = year % 100;
            int m100 = year % 100;

//          int m100 = year - 100; // mutant
            int m400 = year % 400;
//          int m400 = year + 400; // mutant
            if ((m4 != 0) || ((m100 == 0) && (m400 != 0)))
//          if ((m4 != 0) && ((m100 == 0) && (m400 != 0))) // 4.COR –– Conditional Operator Replacement
//          if ((m4 = 0) || ((m100 == 0) || (m400 != 0))) // 9. UOD –– Unary Operator Deletion
...
//          numDays = -(day2 + (daysIn[month1] - day1));  // 8.UOI –– Unary Operator Insertion
//          numDays += day2 + (daysIn[month1] - day1);  // 7.ASR –– Assignment Operator Replacement
//          numDays = day1 + (daysIn[month1] - day1);  // 10. SVR –– Scalar Variable Replacement


            // add the days in the intervening months
            for (int i = month1 + 1; i <= month2-1; i++)
//          for (int i = month1 + 1; i > month2-1; i++)   // 3.ROR –– Relational Operator Replacement
                numDays = daysIn[i] + numDays;
//              throw new RuntimeException("Boom");   //11. BSR –– Bomb Statement Replacement
        }
        return (numDays);
    }
